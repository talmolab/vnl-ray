...
def render_with_rewards(env, policy, observation_spec, rollout_length=150, render=False):
    """
    ...
    """
    frames, reset_idx, reward_channels, activation_collection, kinematics_collection = render_with_rewards_info(env, policy, observation_spec, rollout_length=rollout_length, render=render)
...
def render_with_rewards_info(env, policy, observation_spec, rollout_length=150, render_vision_if_available=False, render=False):
    """
...
def run_simulation(learner, actuator_type, render=False):
    """
    ...
    for episode in tqdm(range(num_episodes)):
        ...
        frames, activation_collection, kinematics_collection = render_with_rewards(
            env, policy, observation_spec, rollout_length=rollout_length, render=render
        )
...
@hydra.main(
    version_base=None,
    config_path="./config",
    config_name="train_config_mouse_reach_offline_position",
)
def main(config: DictConfig) -> None:
    ...
    all_activations_per_episode, all_kinematics_per_episode = run_simulation(learner, actuator_type, render=False)

    # Get the current date and time
    datetime_str = datetime.now().strftime("%Y%m%d_%H%M%S")

    # Paths to save the data with datetime included
    activations_h5_file_path = f"/root/vast/eric/vnl-ray/training/activations/activations_test_{actuator_type}_{datetime_str}.h5"
    kinematics_h5_file_path = f"/root/vast/eric/vnl-ray/training/kinematics/kinematics_test_{actuator_type}_{datetime_str}.h5"
