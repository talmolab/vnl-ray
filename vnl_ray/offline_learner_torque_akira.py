...
def render_with_rewards_info(
    env, policy, observation_spec, rollout_length=150, render_vision_if_available=False, render=False
):
    """
...
def instantiate_learner(config):
    ...
    if config.learner_network["use_intention"]:
        network_factory = make_network_factory_dmpo_intention(
            ...
            use_tfd_independent=config.learner_network["use_tfd_independent"],  # for easier KL calculation
...
@hydra.main(
    version_base=None,
    config_path="./config",
    config_name="train_config_mouse_reach_offline_torque_akira",
)
def main(config: DictConfig) -> None:
    ...
    all_activations_per_episode, all_kinematics_per_episode = run_simulation(learner, actuator_type, render=True)
